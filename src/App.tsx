...
export default function App({ defaultView = 'home' }: { defaultView?: 'home' | 'admin' }) {
  ...
  const fetchRegisteredUsers = async () => {
    const url = API_BASE_URL + '/api/admin/accounts';
    try {
  ...
  const handleConnectGoogle = async () => {
    ...
    try {
      ...
      if (error?.code === 'auth/unauthorized-domain' || (error?.message && error.message.includes('unauthorized-domain'))) {
        alert("Sign in error:\nFirebase: Error (auth/unauthorized-domain).\n\nPlease add this app's domain to your Firebase Authorized Domains in the Firebase Console (Authentication -> Settings -> Authorized Domains).");
      } else {
